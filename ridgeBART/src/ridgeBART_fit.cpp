...
// [[Rcpp::export(".ridgeBART_fit")]]
Rcpp::List ridgeBART_fit(
  ...
  double branch_alpha, double branch_beta, double sigma0,
  ...
  )
{
  ...
  // Rcpp::Rcout << "lil adjustment will be " << 0.5 * (log(tree_pi.det_V) + tree_pi.bVb) << std::endl;
  ...
  int nid; // for holding the node id of where a changed was made used for writing change log and updating length scale prior
  ...
  for(int iter = 0; iter < total_draws; iter++){
    ...
    rho_diag.reset(); // reset running counts of accepted and pruned length scales
    for(int m = 0; m < M; ++m){
      for(suff_stat_it ss_it = ss_train_vec[m].begin(); ss_it != ss_train_vec[m].end(); ++ss_it){
