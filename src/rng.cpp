...
arma::vec RNG::unif_vec(int d, double x, double y)
{
  arma::vec results(d);
  for(int i = 0; i < d; i++) results(i) = uniform(x, y);
  return(results);
}
...
arma::vec RNG::std_laplace_vec(int d)
{
  arma::vec results(d);
  for(int i = 0; i < d; i++) results(i) = laplace(0.0,1.0);
  return(results);
}

arma::vec RNG::std_cauchy_vec(int d)
{
  arma::vec results(d);
  for(int i = 0; i < d; i++) results(i) = cauchy(0.0,1.0);
  return(results);
}
...
arma::mat RNG::std_laplace_mat(int nrow, int ncol)
{
  arma::mat results(nrow, ncol);
  for(int i = 0; i < nrow; i++){
    for(int j = 0; j < ncol; j++) results(i,j) = laplace(0.0, 1.0);
  }
  return(results);
}

arma::mat RNG::std_cauchy_mat(int nrow, int ncol)
{
  arma::mat results(nrow, ncol);
  for(int i = 0; i < nrow; i++){
    for(int j = 0; j < ncol; j++) results(i,j) = cauchy(0.0, 1.0);
  }
  return(results);
}
...
void RNG::unif_direction(std::map<int,double> &phi, int dim)
{
  std::vector<double> tmp_z;
  double ss = 0.0;
  for(int d = 0; d < dim; ++d){
    tmp_z.push_back(normal(0.0,1.0));
    ss += pow(tmp_z[d], 2.0);
  }
  phi.clear();
  double norm = sqrt(ss);
  for(int d = 0; d < dim; ++d){
    phi.insert(std::pair<int,double>(d, tmp_z[d]/norm));
  }
}
