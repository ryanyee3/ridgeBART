...
train_df = data.frame(Z_mat_train, make = Y_train)

# testing data
Y_test = as.integer(Y[test_index])
Z_mat_test = as.matrix(std_z_mat[test_index,])
test_df = data.frame(Z_mat_test, make = Y_test)

# fit model
fit = mgcv::gam(make ~ s(baseline, sideline), family = binomial(), data = train_df)
...
# save
name = paste0("league_gam_", test_fold)
assign(name, results)
